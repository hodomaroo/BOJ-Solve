...
#최소 / 최대
segmentTreeNodes = [-float("inf") for _ in range(pow(2,depth))]
...
def updateValue(targetNode : int, value : int): #갱신하면서 올라가기
    ...
    segmentTreeNodes[targetNode] = value
    ...
    while targetNode:
        ...
        segmentTreeNodes[targetNode // 2] = min(segmentTreeNodes[targetNode],segmentTreeNodes[brotherNode])
...
def getAreaSum(node : int, start: int, end: int, wantLeft: int, wantRight: int) -> int:
    ...
    if wantLeft <= mid:
        minValue = min(minValue,getAreaSum(node * 2, start, mid, wantLeft, min(wantRight, mid)))


    if wantRight > mid:
        minValue = min(minValue,getAreaSum(node * 2 + 1, mid + 1, end, max(mid + 1, wantLeft), wantRight))
